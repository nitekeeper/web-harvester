...
A reusable click-to-edit inline text label for presentation components.

| Name                 | Kind      | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| -------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `EditableLabelProps` | Interface | Props for `EditableLabel`. `value: string` — current display text. `onCommit: (next: string) => void` — called with the trimmed new value when the user commits (not called on cancel, unchanged value, or empty input). `className?: string` — forwarded to both the static span and the edit input. `style?: React.CSSProperties` — forwarded to both elements; edit-mode component defaults take precedence over caller values. `ariaLabel?: string` — accessible name for the static span's `role="button"`; defaults to `value` if omitted. |
| `EditableLabel`      | Component | Renders a static `<span role="button">` that switches to a focused `<input>` on click or Enter/Space keypress. Enter and blur commit the trimmed value (if non-empty and changed); Escape cancels and restores the original label.                                                                                                                                                                                                                                                                                                               |
